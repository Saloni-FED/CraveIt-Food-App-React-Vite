  ...
  return (
    <>
      <div className="m-2 p-6 box-border h-full flex flex-col transition duration-300  shadow-xl hover:-translate-y-5  ">
        ...
        <h2 className="font-light text-slate-500">{props.restaurant?.cuisines?.join(", ")}</h2>
        ...
        {props.restaurant?.avgRating?.toString(10) < 4 ? (
          <span className="bg-amber-300 inline text-white place-self-end px-3 rounded-sm font-bold">{props.restaurant?.avgRating}</span>
        ) : (
          <span className="bg-lime-600 inline text-white place-self-end px-3 rounded-sm font-bold">{props.restaurant?.avgRating}</span>
        )}
        </span>
      </div>
    </>
